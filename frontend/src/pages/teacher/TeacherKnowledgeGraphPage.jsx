...
import { Select, Spin, Empty, Divider } from 'antd'
...
export default function TeacherKnowledgeGraphPage() {
  ...
  useEffect(() => {
    if (courseId && studentId) {
      const s = students.find(s => s.userId === studentId)
      setStudentName(s?.nickname || s?.username || String(studentId))
      loadGraph()
    }
  }, [studentId, includePrerequisites, unlimitedRelated])
  ...
  const loadGraph = async () => {
    setLoading(true)
    setEmpty(false)
    setGraphData(null)   // 先清空旧图
    try {
      const data = await api.knowledge.userGraph(courseId, studentId, includePrerequisites, unlimitedRelated ? 0 : 25)
  ...
  const filterData = (data, nt, et, kcIds = [], exIds = []) => {
    ...
    const edges   = data.edges.filter(e =>
      et.has(e.edgeType) && nodeIds.has(e.source) && nodeIds.has(e.target)
    )
    return { nodes, edges }
  }
  ...
  const computePositions = (nodes, edges, W, H) => {
    ...
    const prereqR = Math.min(W, H) * 0.60
    const pos = {}
  ...
  const renderGraph = async (data, nt, et, isFullscreen, kcIds = [], exIds = []) => {
    ...
    // 当前选中的学生名，供 nodeLabel 使用
    const sName = students.find(s => s.userId === studentId)?.nickname
      || students.find(s => s.userId === studentId)?.username
      || studentName
    ...
    const gNodes = filtered.nodes.map(n => {
      ...
      return {
        id: n.id,
        nodeType: n.nodeType,
        style: { x: p.x, y: p.y, ...nodeStyle(n) },
      }
    })

    const gEdges = filtered.edges.map((e, i) => ({
      id: `e${i}`, source: e.source, target: e.target,
      edgeType: e.edgeType, label: e.label ?? '',
      style: edgeStyle(e),
    }))
    ...
    graphRef.current = new Graph({
      ...
      node: {
        type: 'circle',
        style: {
          labelText:       d => nodeLabel(d, raw.find(n => n.id === d.id), sName),
  ...
  const nodeStyle = n => {
    if (n.nodeType === 'student') return { fill: '#faad14', lineWidth: 0, size: 58 }
    if (n.nodeType === 'kc')      return { fill: masteryColor(n.masteryLevel ?? 0), lineWidth: 0, size: 18 + Math.round((n.masteryLevel ?? 0) * 22) }
    if (n.nodeType === 'prereq')  return { fill: '#d9d9d9', lineWidth: 1.5, stroke: '#8c8c8c', size: 14 }
    return { fill: exfrColor(n.exfr ?? 0), lineWidth: 0, size: 10 + Math.round((n.exfr ?? 0) * 12) }
  }

  // 老师端：学生节点显示学生名字
  const nodeLabel = (d, raw, sName) => {
    if (!raw) return ''
    if (raw.nodeType === 'student') return sName || '学生'
    return raw.label ?? ''
  }
  ...
  // ── JSX ───────────────────────────────────────────────────────
  const topBar = (
    <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '2px 0', padding: '6px 12px', background: '#fff', borderBottom: '1px solid #f0f0f0' }}>
      {/* 节点类型 chip */}
      {NODE_TYPES.map(nt => (
        <Chip key={nt.key} color={nt.color} label={nt.label} shape="circle"
          active={shownNodeTypes.has(nt.key)} onClick={() => toggleNodeType(nt.key)} />
      ))}
      <span style={{ width: 1, height: 16, background: '#e0e0e0', margin: '0 6px', display: 'inline-block' }} />
      {/* 边类型 chip */}
      {EDGE_TYPES.map(et => (
        <Chip key={et.key} color={et.color} label={et.label} shape="line" dash={et.dash}
          active={shownEdgeTypes.has(et.key)} onClick={() => toggleEdgeType(et.key)} />
      ))}
      ...
      <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        >
          <span style={{ width: 11, height: 11, borderRadius: '50%', flexShrink: 0, background: '#d9d9d9', border: '1.5px solid #8c8c8c', display: 'inline-block' }} />
          关联知识点
        </span>
        ...
        <Select size="small" value={courseId} onChange={setCourseId}
          options={courses.map(c => ({ value: c.id, label: c.name }))} style={{ width: 150 }} />
        <Select size="small" value={studentId}
          onChange={id => setStudentId(id)}
          loading={loadingStudents}
          placeholder="选择学生"
          options={students.map(s => ({ value: s.userId, label: s.nickname || s.username }))}
          style={{ width: 130 }} />
        <IconBtn title="刷新" onClick={loadGraph} loading={loading}><ReloadOutlined /></IconBtn>
      ...
      {graphData && (
        <div style={{ width: '100%', display: 'flex', gap: 10, padding: '4px 2px 2px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <Select mode="multiple" size="small" allowClear placeholder="全部知识点"
            maxTagCount="responsive" value={selectedKcIds} onChange={setSelectedKcIds}
            options={graphData.nodes.filter(n => n.nodeType === 'kc').map(n => ({ value: n.id, label: n.label || n.id }))}
            style={{ flex: 1, minWidth: 160, maxWidth: 380 }} />
          <span style={{ fontSize: 12, color: '#888', whiteSpace: 'nowrap' }}>筛选习题：</span>
          <Select mode="multiple" size="small" allowClear placeholder="全部习题"
            maxTagCount="responsive" value={selectedExIds} onChange={setSelectedExIds}
            options={graphData.nodes.filter(n => n.nodeType === 'exercise').map(n => ({ value: n.id, label: n.label || n.id }))}
            style={{ flex: 1, minWidth: 160, maxWidth: 380 }} />
        </div>
      )}
    </div>
  ...
  const canvas = (
    <div style={{ position: 'relative', background: '#fff', height: graphHeight(fullscreen) }}>
      {loading && (
        <div style={{ position: 'absolute', inset: 0, zIndex: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.75)' }}>
          <Spin description="加载图谱中..." />
        </div>
      )}
      {!studentId && !loading && (
        <Empty description="请选择学生" style={{ paddingTop: '15%' }} />
      )}
      {studentId && empty && !loading && (
        <Empty description={`${studentName} 暂无答题记录`} style={{ paddingTop: '15%' }} />
      )}
      <div ref={containerRef}
        style={{ width: '100%', height: '100%', display: (!studentId || empty) ? 'none' : 'block' }} />
    </div>
  )
...
function Chip({ color, label, shape, dash, active, onClick }) {
  return (
    <span onClick={onClick} style={{
      ...
      opacity: active ? 1 : 0.28, transition: 'opacity .15s',
      userSelect: 'none', fontSize: 12, color: '#333', whiteSpace: 'nowrap',
    }}>
      {shape === 'circle' ? (
...
function IconBtn({ children, title, onClick, loading }) {
  ...
  return (
    <span title={title} onClick={!loading ? onClick : undefined}
